...
## §51. Бесключевой граф рекомендаций (Deezer) + холодный старт (2026-09-14)

**Зачем.** Проект выкладывается публично (github.com/Enixq/cryon) с целью «скачал
— и работает»: поиск, воспроизведение и рекомендации без единого ключа. Поиск/аудио
уже бесключевые (YouTube: официальный API → Bing → yt-dlp; SoundCloud), кросс-
источниковый подбор уже реализован (§30.1). Единственная дыра — **рекомендации**:
онлайн-движок был доступен только при ключе Last.fm, а холодный старт (свежий профиль
без избранного/истории) возвращал пусто. Решение — добавить бесключевой граф
похожести (публичный API Deezer) как источник графа и починить холодный старт.

**Суть изменений.**
- **Deezer как fallback-граф (бесключевой).** Новый клиент
  [internal/services/deezer/client.go](internal/services/deezer/client.go) —
  прямой HTTP к `api.deezer.com`, без ключа/регистрации: `/search/artist` →
  `/artist/{id}/related` (похожие), `/artist/{id}/top` (топ-треки), `/chart`
  (чарт). Deezer — **только граф** (кто на кого похож / топ / чарт); аудио оттуда
  не берём (публичный API отдаёт лишь 30-сек превью). `Match` синтезируется из
  ранга (`rankMatch`: верхний ≈1.0, дальше линейно затухает), т.к. `/related` —
  ранжированный список без числовой близости.
- **Last.fm — приоритет, Deezer — фолбэк.** Кэш-обёртки `similarArtistsCached` /
  `topTracksCached` в [engine.go](internal/recommendations/engine.go) сначала
  пробуют Last.fm (если ключ есть), при пустом результате — Deezer, с маппингом
  `deezer.Artist→lastfm.Artist` / `deezer.Track→lastfm.Track`. Ключи кэша сделаны
  нейтральными (`graph:similar:v1:` / `graph:toptracks:v1:`); пустой чистый ответ
  кэшируется, транзиентные ошибки — нет. Существующие Last.fm-пользователи получают
  прежнее поведение; `content.go` (теговый TF-IDF) и `discovery.go` (2 хопа)
  специфичны для Last.fm и **не тронуты** — без ключа штатно деградируют в
  нейтраль/nil.
- **Засев `buildTaste` из локальной библиотеки.** Добавлен сигнал наравне с
  избранным/плейлистами: уникальные артисты из `LocalTrackList`, вес 1.5,
  `deliberate=true` (файл в коллекции — осознанный выбор, без затухания по времени),
  кап топ-50 по числу треков (чтобы 10k-библиотека не раздула профиль). Новый
  пользователь, только просканировавший папку, получает непустой профиль.
- **Чартовый фолбэк вместо пустого возврата.** При полностью пустом профиле (ни
  библиотеки, ни избранного) и доступном Deezer — кандидаты из `ChartTracks`
  прогоняются через тот же `resolveCandidates` (дедуп `artist|title`, взвешивание
  `sourcePrefs`) и возвращаются. Пусто отдаём только когда и профиль пуст, и Deezer
  недоступен.
- **Ветка выбора** в `Recommendations`: `if e.lastfm.Available()` → `if
  e.OnlineAvailable()` (= Last.fm **или** Deezer), чтобы онлайн-путь работал и
  бесключево. Фолбэк на `offlineRecommendations` сохранён.
- **Разделение статусов бэкенда** ([core/app.go](internal/core/app.go)):
  `RecommendationsAvailable()` → `OnlineAvailable()` (теперь ~всегда true — движок
  бесключевой), `LastFMConnected()` → `LastFMAvailable()` (осталось про ключ
  Last.fm, для тумблера в настройках). Deezer в `registry`/`ListSourceStatus` **не**
  добавляется — это граф, а не проигрываемый источник, в списке источников он
  невидим (правильно).
- **Фронтенд — только копирайт, не логика.** Онбординг (шаг 3) и настройки
  (Last.fm/Yandex помечены «необязательно»), JSDoc в `client.ts` переформулированы
  под «работает из коробки, ключи опциональны». Пустые состояния и клиентский
  фолбэк уже были — наполняются сами после бэкенд-фикса.

**Затронутые файлы.** `internal/services/deezer/*` (новый пакет + тесты),
`internal/recommendations/engine.go` (Deezer-фолбэк, засев локалки, чартовый
фолбэк, `OnlineAvailable`/`LastFMAvailable`), `internal/recommendations/coldstart_test.go`
(новый), `internal/core/app.go` (сборка движка с `deezer.New()`, split статусов),
фронтенд-копирайт (`OnboardingModal.tsx`, `SettingsPage.tsx`, `client.ts`).

**Android.** Кода нет: изменения целиком в общем Go-core (`internal/core`,
`internal/recommendations`, `internal/services/deezer`) и попадают в APK через
`gomobile bind` автоматически (`mobile/go.mod` → `replace Cryon2 => ../`, пакет
`deezer` подтягивается транзитивно через `core`→`recommendations`). Пакет `deezer`
без build-тегов, без CGO, только stdlib + `internal/logging` — новых внешних
зависимостей в `mobile/go.mod`/`go.sum` не появляется. См. [plan-android.md](plan-android.md).

**Итог.** Рекомендации работают из коробки без ключей и остаются
кросс-источниковыми (граф → кандидаты → веерный `SearchAll` → лучший проигрываемый
источник). Last.fm/Yandex/Spotify — необязательные усилители. Холодный старт даёт
непустую выдачу и при пустой библиотеке (чарт), и сразу после сканирования папки
(засев из локалки).

## §52. YouTube: надёжный бесключевой поиск и воспроизведение (InnerTube + дедлайны) (2026-09-14)

**Зачем.** YouTube Music «не всегда искал / не всегда включал музыку» — на Android
и на десктопе. Причина в общем Go-ядре (не в UI), поэтому чинится один раз для
обеих платформ. Разбор горячих путей выявил корневые причины:
- **Поиск** был бесключево на двух хрупких скрейпах: парсинг HTML `ytInitialData`
  (`searchByPage`) + скрейп Bing. Оба ломаются от consent-стены, дрейфа разметки и
  бот-детекта, отдавая **0 результатов без ошибки**. На Android третьего пути нет
  (yt-dlp на телефоне отсутствует) — при сбое обоих скрейпов поиск пустой.
- **Воспроизведение** висело без ошибки. `core.GetAudioStream` резолвил URL на
  `a.ctx` — контекст запуска Wails/мобилы **без дедлайна**, а `kkdai/youtube` по
  умолчанию берёт `http.DefaultClient` (`Timeout: 0`). Зависший запрос (медленный
  TLS под VPN, троттлинг, обрыв) висел вечно → держал singleflight в
  [localserver.go](internal/core/localserver.go) → все Range-запросы `<audio>`
  ждали бесконечно → плеер «буферизовал» молча, без `onerror`. Фолбэк Piped бил в
  3 мёртвых инстанса; на Android yt-dlp-фолбэка нет.

**Суть изменений (всё аддитивно и на stdlib — рабочие пути не тронуты).**
- **InnerTube-поиск** (новый [youtube/innertube.go](internal/services/youtube/innertube.go)):
  `POST youtubei/v1/search` — структурный JSON вместо парсинга HTML, самый надёжный
  бесключевой путь, работает на Android (чистый `net/http`). Разбор переиспользует
  тот же рекурсивный сборщик `videoRenderer` (`collectYouTubeVideoRenderers`), что и
  HTML-страница, — уже покрыт тестами. Встроен в `Search()` **сразу после
  официального API и до** скрейпа страницы/Bing. Ключ InnerTube — публичный
  статический ключ веб-клиента YouTube, **не секрет** (зашит в сами клиенты YT, не
  привязан к аккаунту) — политика «секреты не в сборку» не нарушается.
- **InnerTube-плеер** (новый [audiofetcher/innertube.go](internal/services/audiofetcher/innertube.go)):
  `POST youtubei/v1/player` клиентом **IOS** — отдаёт прямые URL потоков без
  расшифровки сигнатур (n/sig), не требует PoToken, работает на Android. Встроен в
  цепочку фолбэков **перед** Piped: `kkdai (ANDROID_VR) → InnerTube (IOS) → Piped →
  yt-dlp`. Независимый от kkdai путь: если основной ломается, есть второй нативный.
- **Дедлайны против вечного зависания** (корневой фикс «не включает»):
  - `core.GetAudioStream` оборачивает резолв YouTube в `context.WithTimeout(a.ctx,
    40s)` — singleflight/`<audio>` больше не висит вечно; резолв завершается
    успехом или ошибкой, фронтенд получает `onerror`. Стрим качается отдельным
    запросом и этим дедлайном не ограничен (URL — уже строка).
  - `ytClient` получил явный `HTTPClient` с **конечными фазовыми таймаутами**
    (Dial/TLS/ResponseHeader) вместо бесконечного `http.DefaultClient` — ни один
    запрос kkdai (InnerTube/страница/visitorId) не висит вечно.
  - Первичная попытка kkdai ограничена собственным под-дедлайном (22s), чтобы даже
    при её зависании у резолвера остался бюджет на фолбэки в пределах общих 40s.
- **Consent-стена в `searchByPage`.** Добавлены `hl=en&gl=US`, cookie согласия
  `SOCS`+`CONSENT` и десктопный User-Agent — раньше в ЕС/при первом визите YouTube
  отдавал страницу согласия без `ytInitialData`, и парсер молча давал 0. (Скрейп
  теперь вторичен после InnerTube, но фикс — дешёвая страховка.)
- **Обновлён список Piped-инстансов** (3 → 7 актуальных), таймаут на инстанс 12s →
  8s (третичный фолбэк; основной бесключевой путь на Android — kkdai + InnerTube).
- **Исправлены mojibake-строки ошибок** в `fetcher.go` (были «`?? ??????? ...`» —
  битая кириллица из-за неверной кодировки при прошлой правке).

**Затронутые файлы.** `internal/services/youtube/innertube.go` (новый, InnerTube-
поиск), `internal/services/youtube/service.go` (встройка InnerTube в `Search`,
consent-фикс `searchByPage`), `internal/services/audiofetcher/innertube.go` (новый,
InnerTube-плеер IOS), `internal/services/audiofetcher/fetcher.go` (bounded
`HTTPClient` для kkdai, split первичного пути + под-дедлайн, фолбэк-цепочка с
InnerTube, обновление Piped, фикс mojibake), `internal/core/app.go` (дедлайн 40s на
резолв YouTube в `GetAudioStream`).

**Android.** Кода нет: всё в общем Go-core, попадает в APK через `gomobile bind`
автоматически. Новые файлы — чистый stdlib (`net/http`, `encoding/json`), без
build-тегов и без новых внешних зависимостей. **Важно:** InnerTube-поиск и
InnerTube-плеер критичны именно для Android, где yt-dlp/mpv-фолбэков нет — теперь у
телефона есть надёжный структурный поиск и второй нативный аудиопуть. См.
[plan-android.md](plan-android.md).

**Итог.** Поиск: официальный API → **InnerTube** → страница (с обходом consent) →
Bing → yt-dlp. Аудио: kkdai (ANDROID_VR) → **InnerTube (IOS)** → Piped(7) → yt-dlp.
Резолв ограничен по времени на всех уровнях (клиент kkdai, под-дедлайн, общий 40s),
поэтому зависший YouTube больше не оборачивается в вечную тишину плеера, а даёт
ошибку и фолбэк. Все изменения аддитивны — существующие рабочие пути (в т.ч.
Last.fm-регресс из §51) не затронуты.
